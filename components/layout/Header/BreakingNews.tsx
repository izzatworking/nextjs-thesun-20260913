...
export default function BreakingNews({
  ...
  return (
      <div className="w-full bg-[#307EDB] py-2 overflow-hidden relative"
                ...
                <div ref={contentRef} className="flex items-center space-x-6 pr-6">
                  {breakingNews.map((news, index) => (
                    <div key={news.id} className="flex items-center">
                      {news.category && (
                        <span className="bg-white/15 border border-white/30 text-white px-2 py-0.5 rounded text-[10px] font-bold mr-2 whitespace-nowrap">
                          {cleanHtmlContent(news.category)}
                        </span>
                      )}
                      <Link href={news.link} className="text-white hover:text-yellow-300 text-xs font-medium whitespace-nowrap transition-colors">
            ...
            <div className="hidden lg:flex items-center gap-1 ml-3 flex-shrink-0">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="w-6 h-6 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/25 text-white transition-colors"
                  title={social.name}
                >
                  {social.icon}
                </a>
