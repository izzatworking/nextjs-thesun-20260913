...
import { getPostsByTagSlug, getTagBySlug, getTags, getCategories, getPostUrl, setCategoryCache } from '../../lib/wordpress';
...
export const getServerSideProps: GetServerSideProps<TagPageProps> = async (context) => {
  ...
  if (!tag) {
    return { props: { tag: null, posts: [], categories: [], allTags: [] } };
  }

  const [posts, categories, allTags] = await Promise.all([
    getPostsByTagSlug(slug, 100).catch(() => [] as WPPostWithMedia[]),
    getCategories().catch(() => [] as WPCategory[]),
    getTags().catch(() => [] as WPTag[]),
  ]);
